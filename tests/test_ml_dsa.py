...
class TestMLDSA(unittest.TestCase):
    ...
    def generic_test_ml_dsa(self, Dilithium):
    ...
    def test_dilithium2(self):
        for _ in range(5):
            self.generic_test_ml_dsa(MLDSA_128)

    def test_dilithium3(self):
        for _ in range(5):
            self.generic_test_ml_dsa(MLDSA_192)

    def test_dilithium5(self):
        for _ in range(5):
            self.generic_test_ml_dsa(MLDSA_256)
...
class TestMLDSADeterministic(unittest.TestCase):
    ...
    def generic_test_ml_dsa(self, ML_DSA, count=5):
        for _ in range(count):
            msg = b"Signed by dilithium" + secrets.token_bytes(random.randint(1, 1000))
            ctx = secrets.token_bytes(128)

            # Perform signature process
            pk, sk = ML_DSA.ml_dsa_keygen()
            sig = ML_DSA.ml_dsa_sign(sk, ML_DSA.convert.bytes_to_bits(msg), ctx=ctx)
            check_verify = ML_DSA.ml_dsa_verify(pk, ML_DSA.convert.bytes_to_bits(msg), sig, ctx=ctx)

            # Generate some fail cases
            pk_bad, _ = ML_DSA.ml_dsa_keygen()
            check_wrong_pk = ML_DSA.ml_dsa_verify(pk_bad, ML_DSA.convert.bytes_to_bits(msg), sig, ctx=ctx)
            check_wrong_msg = ML_DSA.ml_dsa_verify(pk, ML_DSA.convert.bytes_to_bits(b""), sig, ctx=ctx)
            check_no_ctx = ML_DSA.ml_dsa_verify(pk, ML_DSA.convert.bytes_to_bits(msg), sig)
    ...
    def test_derive_with_wrong_seed_length(self):
        with self.assertRaises(ValueError) as e:
            MLDSA_128.ml_dsa_keygen_internal(bytes(range(31)))

        self.assertIn("expected a 32-bytes bytestring.", str(e.exception))
    ...
    def test_derive_from_seed_ML_DSA_44(self):
        pk, _ = MLDSA_128.ml_dsa_keygen_internal(bytes(range(32)))
    ...
    def test_derive_from_seed_ML_DSA_65(self):
        pk, _ = MLDSA_192.ml_dsa_keygen_internal(bytes(range(32)))
    ...
    def test_derive_from_seed_ML_DSA_87(self):
        pk, _ = MLDSA_256.ml_dsa_keygen_internal(bytes(range(32)))
...
class TestML_DSA_KAT(unittest.TestCase):
    ...
    def generic_keygen_kat(self, ML_DSA, index):
        ...
        for test in kat_data:
            ...
            pk, sk = ML_DSA.ml_dsa_keygen_internal(seed)
    ...
    def generic_sign_kat(self, ML_DSA, index, deterministic=False):
        ...
        for test in kat_data:
            ...
            sig = ML_DSA.ml_dsa_sign_internal(sk_kat, ML_DSA.convert.bytes_to_bits(msg_kat), rng_kat)
    ...
    def generic_verify_kat(self, ML_DSA, index):
        ...
        for test in kat_data:
            ...
            check = ML_DSA.ml_dsa_verify_internal(pk_kat, ML_DSA.convert.bytes_to_bits(msg_kat), sig_kat)
            self.assertEqual(check, check_kat)

    def test_ML_DSA_44_keygen(self):
        self.generic_keygen_kat(MLDSA_128, 0)

    def test_ML_DSA_65_keygen(self):
        self.generic_keygen_kat(MLDSA_192, 1)

    def test_ML_DSA_87_keygen(self):
        self.generic_keygen_kat(MLDSA_256, 2)

    def test_ML_DSA_44_sign(self):
        self.generic_sign_kat(MLDSA_128, 0, deterministic=True)

    def test_ML_DSA_65_sign(self):
        self.generic_sign_kat(MLDSA_192, 1, deterministic=True)

    def test_ML_DSA_87_sign(self):
        self.generic_sign_kat(MLDSA_256, 2, deterministic=True)

    def test_ML_DSA_44_verify(self):
        self.generic_verify_kat(MLDSA_128, 0)

    def test_ML_DSA_65_verify(self):
        self.generic_verify_kat(MLDSA_192, 1)

    def test_ML_DSA_87_verify(self):
        self.generic_verify_kat(MLDSA_256, 2)
